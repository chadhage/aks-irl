# Scenario — Replatform a Real-Time Messaging System onto AKS
...
**Skybridge Messaging Services** is a tier-1 real-time messaging operator. They move ~2.4 billion messages a month between partner systems, edge devices, upstream producers, and downstream consumers. Their flagship product is a real-time messaging bus built around a **custom framed wire protocol** over **persistent TCP sockets**, with a **C++ decode tier reached over HTTPS (REST/JSON)** and strict end-to-end SLAs.
...
| Compliance | PCI-adjacent (payment-card-routing messages), regional data-residency regulations |
| Audit retention | 90 days hot, 7 years cold |

## The legacy estate

```
         Client endpoints (partner systems, devices, services)
                       │
        ...
        │  - Custom wire-format decoder    │   shared memory IPC to gateway
...
2. **Capacity is bi-modal.** Peak-hour traffic is 4× off-peak, but the VMs are sized for peak — ~$1.8M/yr of idle compute.
...
- Changing the on-the-wire protocol. Client endpoints connect exactly the same way; the gateway just lives somewhere new.
...
```
   Client endpoints                           Ops team (browsers)
         │                                          │
...
| **Front Door + WAF only for the ops console.** | Client endpoints connect via L4 — Front Door doesn't add value for raw TCP. NLB + DNS + region-pair failover is the right shape. | M01, M07 |
...
> "If we connect a single non-production client partner to this MVP next Monday, it stays up for a full week of peak-hours traffic, we can ship a parser rule change to canary without a maintenance window, and a planned zone failure doesn't make the on-call phone ring."
